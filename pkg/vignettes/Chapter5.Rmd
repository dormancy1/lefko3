...
#### Step 1. Life history model development 
  ...
  <p style="text-indent: 20px">We will first describe the life history characterizing the population, matching it to our dataset with a *stageframe* for our *Cypripedium candidum* dataset. Since this analysis will be function-based and all sizes have reasonable representation, we will include all observed size classes here. Size is count-based, and refers to all numbers of stems that occurred, representing the life history diagram shown previously. Note that we are using the midpoint approach to determining the size bins here, using the default bin halfwidths of 0.5 (since all size bins are using this default, we do not include a `binhalfwidth` vector option in the `sf_create()` input). However, we could have used the `sizemin` and `sizemax` options to set the size bin minima and maxima instead.</p>
...
#### Step 2c. Provide supplemental information for matrix estimation  
  ...
  5. additive offsets to survival or fecundity transitions.
...
```
  ...
  <p style="text-indent: 20px">In the output, we do not see historical terms in any best-fit model, suggesting that we can ignore history. We can also see that although the accuracy of our some vital rate models is very high, while it was only fair for others. For comparison, we may wish to estimate ahistorical models. In that case, we also need linear models in which the global models tested do not include state at occasion *t*-1, as below. (NOTE: The size distribution utilized incorporates some randomization during the modeling process, which may result in the incorporation of a historical term in the best-fit model of size.)</p>
...
which(as.matrix(tm3sens_s$h_sensmats[[1]]) == 
  max(as.matrix(tm3sens_s$h_sensmats[[1]])[which(as.matrix(tmeans3r$A[[1]]) > 0)]))
#>  [1] 962604 962658 962712 962766 962820 962874 962928 962982 963036 963090 963144 963198 963252 963306 963360 963414 963468 963522 963576
#> [20] 963630 963684 963738 963792 963846 963900 963954 964008 964062 964116 964170 964224 964278 964332 964386 964440 964494 964548 964602
#> [39] 964656 964710 964764 964818 964872 964926 964980 965034 965088 965142 965196
...
![Figure 5.10. Projected *Cypripedium* population dynamics against boosted fecundity](Ch5.28-1.png)
