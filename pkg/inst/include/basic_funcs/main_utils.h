...
namespace LefkoUtils {
  ...
  inline bool yesno_to_logic (RObject input, String arg_name) {
    ...
    if (is<StringVector>(input)) {
      ...
      for (int i = 0; i < 8; i++) {
        if (stringcompare_simple(input_check, String(yesbits(i)), true)) yes_check++;
        if (stringcompare_simple(input_check, String(nobits(i)), true)) no_check++;
      }
  ...
  inline void yesnoauto_to_logic (RObject input, String arg_name,
    ...
    if (is<StringVector>(input)) {
      ...
      for (int i = 0; i < 8; i++) {
        if (i < 5) {
          if (stringcompare_simple(input_check, String(autobits(i)), true)) auto_check++;
        }
        if (stringcompare_simple(input_check, String(yesbits(i)), true)) yes_check++;
        if (stringcompare_simple(input_check, String(nobits(i)), true)) no_check++;
      }
